...
def motif(device, model, state_dict, test_loader, outdir, thres):
    ...
    for i_batch, sample_batch in enumerate(test_loader):
        ...
        for i in range(kernel_num):
            index = max_index[i]
            index += start
            data_slice = data[[0, 1, 2, 3], index:(index + motifLen)]
...
def main():
    ...
    for cv in range(1):
        ...
        # model = FCNARRB(motiflen=motifLen, channels=4)
        model = FCNARRB(motiflen=motifLen, channels=5)
