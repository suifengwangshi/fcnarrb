...
class Trainer(object):
    ...
    def train(self):
        ...
        return self.piou_best, self.niou_best, self.iou_best, self.prauc_best, self.state_best
    ...
    def test(self):
        ...
        for i_batch, sample_batch in enumerate(self.test_loader):
            ...
            seg_v_all.append(denselabel_p.view(-1).data.cpu().numpy())
            seg_t_all.append(denselabel.view(-1).data.cpu().numpy())

        iou, prauc, piou, niou = label_accuracy_score(seg_t_all, seg_p_all, seg_v_all, n_class=2)
        if self.iou_best < iou or self.prauc_best < prauc:
            self.piou_best = piou
            self.niou_best = niou
            self.iou_best = iou
            self.prauc_best = prauc
            self.state_best = deepcopy(self.model.state_dict())
        print("piou: {:.3}, niou: {:.3}, iou: {:.3f}, prauc: {:.3}\n".format(piou, niou, iou, prauc))
