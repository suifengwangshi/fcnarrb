...
from FCNmotif import FCN, FCNA, FCNARRB
...
def get_args():
    ...
    parser.add_argument("-rl", dest="ratio_loss", type=float, default=0.3,
                        help="ratio for negative samples.")
    parser.add_argument("-ch", dest="data_channels", type=int, default=4,
                        help="Whether add conservation information")
...
def main():
    ...
    f.write('CV\ta1\ta2\tPIOU\tNIOU\tIOU\tPRAUC\n')
    piou_mean = 0.
    niou_mean = 0.
    iou_mean = 0.
    prauc_mean = 0.
    for cv in range(cv_num):
        ...
        for trial in range(5):
            # model = FCN(motiflen=motifLen, channels=4)
            # model = FCNA(motiflen=motifLen, channels=4)
            model = FCNARRB(motiflen=motifLen, channels=args.data_channels)
            optimizer = optim.Adam(model.parameters(),
                                   lr=args.learning_rate,
                                   weight_decay=args.weight_decay)
            criterion = OhemNegLoss(device, args.ratio_loss)
            start_epoch = 0
            if args.restore:
            ...
            piou, niou, iou, prauc, state_dict = executor.train()
            if iou_best < iou or prauc_best < prauc:
                if piou_best < piou:
                    piou_best = piou
                if niou_best < niou:
                    niou_best = niou
        ...
        f.write("{}\t{}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\n".format(cv, trial_best, piou_best, niou_best, iou_best, prauc_best))
        f.flush()
    f.write("mean\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\n".format(piou_mean/cv_num, niou_mean/cv_num, iou_mean/cv_num, prauc_mean/cv_num))
    f.close()
